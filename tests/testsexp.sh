...
# Test script for the S-expression grammar with a configurable backend.
...
echo "Testing S-expression grammar with backend '$backend' and length $length..."

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
GRAMMAR="$REPO_ROOT/grammars/sexp.alp"
...
run_sexp_test() {
    ...
    echo "========================================="
    echo "Testing S-expression $mode_name mode (backend=$backend)..."
    echo "========================================="
    ...
    if [ "$backend" = "c" ]; then
        ...
        futhark script --backend="$backend" -b sexp.fut 'test ($loadbytes "sexp.inputs")' \
            | tail -c +16 > sexp.results
    fi

    # shellcheck disable=SC2086
    if ! alpacc test compare "$GRAMMAR" sexp.inputs sexp.outputs sexp.results $mode_flag; then
        echo "ERROR: Test FAILED for S-expression $mode_name mode"
        return 1
    fi

    echo "S-expression $mode_name test PASSED"
    return 0
}

run_sexp_test "--lexer"  || exit 1
run_sexp_test "--parser" || exit 1
run_sexp_test ""         || exit 1

echo "All S-expression grammar tests passed."
