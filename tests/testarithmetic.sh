...
# Test script for the arithmetic grammar with a configurable backend.
...
echo "Testing arithmetic grammar with backend '$backend' and length $length..."

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
GRAMMAR="$REPO_ROOT/grammars/arithmetic.alp"
...
run_arithmetic_test() {
    ...
    echo "========================================="
    echo "Testing arithmetic $mode_name mode (backend=$backend)..."
    echo "========================================="
    ...
    if [ "$backend" = "c" ]; then
        ...
        # shellcheck disable=SC2086
        if ! alpacc c "$GRAMMAR" $mode_flag -o arithmetic.c; then
            echo "ERROR: alpacc c failed for $mode_name mode"
            return 1
        fi

        if ! cc -std=c99 -O2 -o arithmetic arithmetic.c &>/dev/null; then
            echo "ERROR: cc compilation failed for $mode_name mode"
            return 1
        fi

        ./arithmetic < arithmetic.inputs > arithmetic.results

    elif [ "$backend" = "cuda" ]; then
        # CUDA backend: compile the generated .cu file with nvcc and run it.
        # shellcheck disable=SC2086
        if ! alpacc cuda "$GRAMMAR" $mode_flag -o arithmetic.cu; then
            echo "ERROR: alpacc cuda failed for $mode_name mode"
            return 1
        fi

        local arch="${CUDA_ARCH:-native}"
        if ! nvcc -O3 -std=c++17 -arch="$arch" -o arithmetic arithmetic.cu &>/dev/null; then
        ...
        futhark script --backend="$backend" -b arithmetic.fut 'test ($loadbytes "arithmetic.inputs")' \
            | tail -c +16 > arithmetic.results
    fi

    # shellcheck disable=SC2086
    if ! alpacc test compare "$GRAMMAR" arithmetic.inputs arithmetic.outputs arithmetic.results $mode_flag; then
        echo "ERROR: Test FAILED for arithmetic $mode_name mode"
        return 1
    fi

    echo "Arithmetic $mode_name test PASSED"
    return 0
}

run_arithmetic_test "--lexer"  || exit 1
run_arithmetic_test "--parser" || exit 1
run_arithmetic_test ""         || exit 1

echo "All arithmetic grammar tests passed."
