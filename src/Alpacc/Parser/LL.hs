...
generateRandomDerivationFold gen targetLen grammar step z =
  let prod_map  = toProductionsMap $ productions grammar
   ...
   in deriveFold expandTable contractTable gen 0 (Seq.singleton (Nonterminal $ ntIdx Map.! start grammar)) z
  where
    -- Cap the Seq at ~100k symbols to bound peak memory.
    -- When the queue is full, force contracting choices until it drains.
    maxQueueSize = 100000 :: Int
       ...
       in if snd (bounds allArr) < 0
            then deriveFold expandTable contractTable g termCount syms acc
              ...
              let remaining    = targetLen - termCount
                  queueFull    = Seq.length syms >= maxQueueSize
                  shouldExpand = not queueFull && remaining > 5 && snd (bounds recArr) >= 0
                  !(g', rhs) =
                    if shouldExpand
